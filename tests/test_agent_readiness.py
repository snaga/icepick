...
class TestAgentReadiness:
    ...
    def test_readiness_no_hang_on_non_interactive_stdin(self, tmp_path: Path) -> None:
        ...
        # 1. Test fix --interactive with empty/EOF stdin stream
        t_start = time.perf_counter()
        with patch("sys.stdin", io.StringIO("")):
            result_interactive = runner.invoke(
                app, ["fix", str(sql_file), "--interactive"], input=""
            )
        ...
        # 2. Test fix batch execution with non-interactive stdin
        t_start = time.perf_counter()
        with patch("sys.stdin", io.StringIO("")):
            result_batch = runner.invoke(app, ["fix", str(sql_file)], input="")
        elapsed_batch = time.perf_counter() - t_start

        assert elapsed_batch < 2.0, f"Batch fix took too long ({elapsed_batch:.2f}s)"
    ...
    def test_readiness_all_commands_json_parseable(self, tmp_path: Path) -> None:
        ...
        dirty_sql.write_text(
            "SELECT * FROM (SELECT id FROM tbl ORDER BY id) AS sub WHERE DATE(created_at) = '2023-01-01'",
            encoding="utf-8",
        )
        ...
        # 3. fix --json (--dry-run)
        res_fix = runner.invoke(app, ["fix", str(dirty_sql), "--json", "--dry-run"])
        assert res_fix.exit_code == 0
        data_fix = _assert_valid_json_and_no_ansi(res_fix.stdout)
        assert isinstance(data_fix, dict)
        assert data_fix["status"] == "optimized"
        assert data_fix["file"] == str(dirty_sql)
        assert "issues_fixed" in data_fix
        assert "diff" in data_fix
        assert len(data_fix["diff"]) > 0
    ...
    def test_readiness_enumerated_error_on_invalid_arguments(self, tmp_path: Path) -> None:
        ...
        # 2. Invalid dialect in 'fix' command
        res_fix_bad_dialect = runner.invoke(
            app, ["fix", str(sql_file), "--dialect", "unsupported_dialect"]
        )
    ...
    def test_readiness_safe_mutation_boundary(self, tmp_path: Path) -> None:
        ...
        original_sql = "SELECT * FROM (SELECT id FROM tbl ORDER BY id) AS sub\n"
        sql_file.write_text(original_sql, encoding="utf-8")
        patch_file = tmp_path / "test.patch"

        # 1. Attempt --write without --force in non-interactive environment (sys.stdin.isatty() is False)
        with patch("sys.stdin.isatty", return_value=False):
            res_write_no_force = runner.invoke(app, ["fix", str(sql_file), "--write"])

        assert res_write_no_force.exit_code == 1
        assert (
            "error: Overwriting files in non-interactive environment requires --force flag"
            in res_write_no_force.stderr
        )
        # File must remain completely untouched
        assert sql_file.read_text(encoding="utf-8") == original_sql

        # 2. Attempt --write and --patch with --dry-run (simulation only)
        ...
        with patch("sys.stdin.isatty", return_value=False):
            res_write_force = runner.invoke(app, ["fix", str(sql_file), "--write", "--force"])

        assert res_write_force.exit_code == 0
