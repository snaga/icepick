tech_content = """# テクノロジースタック (tech.md)
...
- **`typer`** (または `click`):
  - サブコマンド（`check`, `fix`, `diff`, `verify`）およびオプションフラグ管理。
...
### 4. 外部API / LLM連携
- **`litellm`** (または `anthropic` / `openai` SDK):
  - 局所スライスされたサブクエリリライト用のLLMプロバイダ抽象化（Claude 3.5 Sonnet / GPT-4o 等の切り替え）。
...
with open('Temp/Snowflake_Query_Optimizer_PoC/SPECS/tech.md', 'w', encoding='utf-8') as f:
    f.write(tech_content.strip() + '\n')
print('Successfully created tech.md')
