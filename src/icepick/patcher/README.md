...
- **外科手術的 In-place 置換 (`in_place.ASTPatcher`)**:
  - `issue.suggested_replacement` を用いて、対象ノード（`issue.target_node`）のみをその位置で差し替える（`SNOW-001`, `SNOW-004`, `SNOW-006`, `SNOW-009` 等）。
  - 親ノード、兄弟ノード、未変更の節（SELECTリスト、JOIN条件、エイリアス等）の構文構造とセマンティクスを100%維持する。
- **ノード安全削除 (`pop()`)**:
  - 不要ソート（`SNOW-003`）や集計ブロック内の冗長 DISTINCT（`SNOW-008`）のように削除が推奨されるノードを、親クエリから安全に除去する。
- **サブクエリ平坦化 (`subquery_to_cte.SubqueryToCTE`)**:
  - FROM/JOIN 句にネストしたインラインサブクエリ（`SNOW-007`）を命名されたトップレベル CTE に昇格させ、元の参照箇所をシンプルなテーブル参照に差し替える。
- **元コード書式保持型局所置換 (`splicer.TextSplicer`)**:
  - 元の SQL ファイルのインデント、改行、大文字小文字、インラインコメントを 100% 保持したまま患部のみを局所置換。
  - コメント内部（`--`、`/* */`）のキーワード誤マッチ除外や、`AS` 有無の差異を許容する柔軟な正規表現マッチングを完備。
