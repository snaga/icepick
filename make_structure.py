structure_content = """# プロジェクト構造 (structure.md)

## 📁 フォルダ構成

```text
Temp/Snowflake_Query_Optimizer_PoC/
├── SPECS/                      # SDD 仕様書群
│   ├── product.md              # 製品概要・ゴール
│   ├── tech.md                 # 技術スタック
...
- プロジェクト内モジュールは絶対インポートを使用する：
  ```python
  from snow_opt.parser import parse_snowflake_sql
  from snow_opt.linter.base import DiagnosticIssue, BaseRule
  ```
"""

with open('Temp/Snowflake_Query_Optimizer_PoC/SPECS/structure.md', 'w', encoding='utf-8') as f:
    f.write(structure_content.strip() + '\n')
print('Successfully created structure.md')
