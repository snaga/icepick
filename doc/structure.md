...
│       │       └── snow_012_select_star.py            # 不要な全列展開警告
...
- **ルールID**: `SNOW-XXX` (3桁の連番プレフィックス: `SNOW-001` 〜 `SNOW-012`)

## 🏗️ アーキテクチャの方針
- **単一責任の原則 (SRP)**:
  - 構文パース (`parser.py`)、診断 (`linter/`)、処方箋管理 (`prescription/`)、置換 (`patcher/`)、差分提示 (`diff/`)、等価性検証SQL生成 (`verifier/`) を完全に疎結合に分離する。
- **純粋関数の徹底 (Purity & Determinism)**:
  - AST診断および置換ロジック、検証SQL生成は副作用を持たない決定論的純粋関数として実装し、外部通信を行わない完全ローカル動作とする。
- **Fail-Safe設計**:
  - ルールが生成したパッチコードは必ず `sqlglot.parse_one` で事前バリデーションを行い、構文エラーが発生した場合は元のASTノードを絶対に破壊せず安全にフォールバックする。
- **完全ゼロ・クレデンシャル設計 (ADR-0004, ADR-0007)**:
  - データベース直接実行および外部 LLM 呼び出しを一切行わず、認証情報（DBパスワードやAPIキー）を保持・要求しない。外部実行は公式 `snow CLI`、高度なリライトは呼び出し元 AI エージェント（および探索 CLI `ast-digger`）に委譲する。
- **Layer 1 Intent の永続化**:
...
- プロジェクト内モジュールは絶対インポートを使用する：
  ...
  from icepick.prescription import PrescriptionEngine
  ```
...
- **ADR による意思決定の永続化 (`doc/adr/`)**:
  ...
  - ADR-0005: 処方箋ファーストアーキテクチャの導入と診断・適用の責任分離
  - ADR-0006: レガシーコマンド（check, rewrite, patch）の削除と処方箋駆動パイプラインへの純化
  - ADR-0007: LLM 連携の完全委譲と純粋決定論的 AST ツールへの純化
